package br.com.aws.projeto01.config.local;
...
public class SnsCreate {
    ...
    public SnsCreate() {
        this.snsClient = AmazonSNSClient.builder()
                .withClientConfiguration(new AwsClientBuilder
                        .EndpointConfiguration("http://localhost:4566",
                            Regions.US_EAST_1.getName()))
                .withCredentials(new DefaultAwsRegionProviderChain())
                .build();

        CreateTopicRequest createTopicRequest = new CreateTopicRequest("product-events");
        this.productEventsTopic = this.snsClient.createTopic(createTopicRequest);

        LOG.info("SNS topic AWS : ${} ", this.productEventsTopic);
    }
